...
class ShmSize(RockerExtension):
    ...
    def _validate_memory_format(value):
        ...
        if not re.match(r'^\d+[kmgKMG]?$', value):
            ...
            raise ArgumentTypeError(f"Invalid memory format: {value}. Expected format: number[k|m|g]")
        return value
    ...
    def register_arguments(parser, defaults={}):
        parser.add_argument('--shm-size',
                            type=ShmSize._validate_memory_format,
...
class MemoryLimits(RockerExtension):
    ...
    def register_arguments(parser, defaults={}):
        parser.add_argument('--memory', '-m',
                            type=MemoryLimits._validate_memory_format,
